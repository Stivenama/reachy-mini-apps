...
export const getListenState = () => rpcCall("conversation.listen", {});
export const setListening = (listening) => rpcCall("conversation.listen", { listening });

export const newSession = () => rpcCall("conversation.new_session", {});
export const readText = (text) => rpcCall("conversation.read", { text });
...
/** Timeline (línea de tiempo de emociones) — endpoints HTTP de /api/timeline. */
const TIMELINE_TIMEOUT_MS = 15000;

async function timelineFetch(path, options = {}) {
  const response = await fetch(`/api/timeline/${path}`, {
    headers: { "Content-Type": "application/json" },
    ...options,
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data?.error || `timeline error ${response.status}`);
  }
  return data;
}

export const timelineEmotions = () =>
  timelineFetch("emotions", { timeout: TIMELINE_TIMEOUT_MS });
export const timelineState = () => timelineFetch("state");
export const timelineConfigure = (duration) =>
  timelineFetch("configure", { method: "POST", body: JSON.stringify({ duration }) });
export const timelineAddEvent = (time, emotion) =>
  timelineFetch("events", { method: "POST", body: JSON.stringify({ time, emotion }) });
export const timelineRemoveEvent = (id) =>
  timelineFetch("events", { method: "DELETE", body: JSON.stringify({ id }) });
export const timelinePlay = () => timelineFetch("play", { method: "POST" });
export const timelinePause = () => timelineFetch("pause", { method: "POST" });
export const timelineStop = () => timelineFetch("stop", { method: "POST" });
export const timelineSaved = () => timelineFetch("saved");
export const timelineSave = (name) =>
  timelineFetch("save", { method: "POST", body: JSON.stringify({ name }) });
export const timelineLoad = (name) =>
  timelineFetch("load", { method: "POST", body: JSON.stringify({ name }) });
export const timelineDelete = (name) =>
  timelineFetch("saved", { method: "DELETE", body: JSON.stringify({ name }) });
