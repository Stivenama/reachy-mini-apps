...
export function createOrb({ initialState = ORB_STATES.IDLE, onStateChange, holdStates = [] } = {}) {
  ...
  function bumpIdleTimer(state) {
    ...
    // Held states persist until the backend releases them (e.g. push-to-talk listening).
    if (holdStates.includes(state)) return;
    // Only LISTENING falls back on silence: THINKING/SPEAKING stay until a
    // terminal event (assistant_transcript_done / response.done) returns the
    // orb to idle, so the mic never reappears while the model is still working.
    if (state !== ORB_STATES.LISTENING) return;
