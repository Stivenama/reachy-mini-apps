...
class RemoteToolSpec:
    ...
    def from_mcp_tool(cls, server_alias: str, tool: "McpTool") -> "RemoteToolSpec":
        ...
        # mcp >= 2.0 renamed inputSchema to input_schema; read both spellings.
    ...
    def from_call_tool_result(
        ...
        *,
        ...
        return cls(
            ...
            status="error" if bool(is_error) else "ok",
...
class RemoteMcpToolClient:
        ...
        try:
            async with self._session() as session:
                result = await session.call_tool(
                    ...
                    # Seconds as a float: mcp >= 2.0 sums it with a float
                    # deadline, while older versions accept either.
                    read_timeout_seconds=float(self.server.tool_timeout_s),
                )
        except McpDependencyError:
            raise
    ...
    async def _list_all_tools(self, session: "ClientSession") -> list["McpTool"]:
        ...
        while True:
            ...
            # mcp >= 2.0 renames nextCursor to next_cursor; read both spellings.
    ...
    async def _list_tools_page(session: "ClientSession", cursor: str | None) -> Any:
        """List one page of tools, adapting to the installed mcp SDK's API."""
        try:
            return await session.list_tools(cursor=cursor)
        except TypeError:
            # mcp >= 2.0 replaced the cursor kwarg with pagination params.
            return await session.list_tools(params={"cursor": cursor} if cursor else None)
    ...
    async def _session(self) -> AsyncIterator["ClientSession"]:
        ...
        async with httpx.AsyncClient(
            ...
            async with streamable_http_client(self.server.url, http_client=http_client) as transport:
                # mcp >= 2.0 yields (read, write); older versions also yield a
                # terminate event as a third element.
                if len(transport) == 2:
