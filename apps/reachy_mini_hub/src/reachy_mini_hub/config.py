...
def _is_source_checkout_root(root: Path) -> bool:
    ...
    return (root / "pyproject.toml").is_file() and (root / "src" / "reachy_mini_hub").is_dir()
...
PUSH_TO_TALK_ENV = "REACHY_MINI_PUSH_TO_TALK"
...
def push_to_talk_enabled() -> bool:
    """Return whether push-to-talk mode is active (default: on)."""
    return _env_flag(PUSH_TO_TALK_ENV, default=True)
