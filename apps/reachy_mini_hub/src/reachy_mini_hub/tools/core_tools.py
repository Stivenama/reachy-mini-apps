...
from reachy_mini_hub.config import config, list_tool_module_names
from reachy_mini_hub.mcp_client import McpToolTimeoutError, McpToolInvocationError
from reachy_mini_hub.tool_spaces import build_remote_client, read_installed_tool_spaces
from reachy_mini_hub.profile_toolsets import read_profile_tool_names
from reachy_mini_hub.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_mini_hub.mcp_client import RemoteMcpToolClient
...
_EXTERNAL_TOOL_MODULE_NAMESPACE = "reachy_mini_hub._external_tools"
...
def _load_enabled_tools(tool_names: list[str], remote_tool_names: set[str]) -> List[type[Tool]]:
    ...
    for tool_name in tool_names:
        if tool_name in remote_tool_names:
            logger.info("Ã¢Å“â€œ Registered remote tool: %s", tool_name)
            continue

        shared_module_path = f"reachy_mini_hub.tools.{tool_name}"
        try:
            ...
            if source == "file":
                logger.info("Ã¢Å“â€œ %s external tool: %s", action, tool_name)
            else:
                logger.info("Ã¢Å“â€œ %s core tool: %s", action, tool_name)
        except (ModuleNotFoundError, FileNotFoundError):
            logger.warning("Ã¢Å¡Â Ã¯Â¸Â Tool '%s' not found in shared or external tools", tool_name)
        except Exception as e:
            logger.error("Ã¢ÂÅ’ Failed to load tool '%s': %s", tool_name, _format_error(e))
