...
from reachy_mini_hub.streaming import AdditionalOutputs, AsyncStreamHandler, wait_for_item
from reachy_mini_hub.idle_policy import start_idle_tool_call
from reachy_mini_hub.tools.core_tools import ToolDependencies, get_tool_specs
from reachy_mini_hub.tools.background_tool_manager import BackgroundToolManager
...
class ConversationHandler(AsyncStreamHandler, ABC):
    ...
    # Set by the LocalStream while the user holds the push-to-talk button.
    push_to_talk_holding: bool = False
    _activity_observer: Callable[[str], None] | None = None
    _transcript_observer: Callable[[str, str, bool], None] | None = None
    _notice_observer: Callable[[str, str], None] | None = None
    _text_observer: Callable[[str], None] | None = None
    ...
    def set_notice_observer(self, observer: Callable[[str, str], None] | None) -> None:
        """Attach/detach a user-facing notice observer, called (kind, message)."""
        self._notice_observer = observer

    def set_text_observer(self, observer: Callable[[str], None] | None) -> None:
        """Attach/detach an assistant text observer, called with the final text."""
        self._text_observer = observer

    def _emit_text(self, text: str) -> None:
        """Forward one final assistant text chunk to the observer, if attached."""
        observer = self._text_observer
        if observer is not None and text:
            try:
                observer(text)
            except Exception:
                logger.debug("text observer raised (ignored)", exc_info=True)

    def _emit_notice(self, kind: str, message: str) -> None:
        """Forward one user-facing notice to the observer, if attached."""
        observer = self._notice_observer
        if observer is not None:
            try:
                observer(kind, message)
            except Exception:
                logger.debug("notice observer raised (ignored)", exc_info=True)
    ...
    async def commit_input(self) -> None:
        """Commit buffered user audio to trigger a response (push-to-talk)."""
        return
