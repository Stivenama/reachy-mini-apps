...
class BreathingMove(Move):  # type: ignore
    ...
    def __init__(
        ...
        self.neutral_antennas = np.array([-0.1745, 0.1745])  # ~10Ã‚Â° offset to reduce shaking
...
class MovementManager:
    ...
    def _get_primary_pose(self, current_time: float) -> FullBodyPose:
        ...
        if self.state.current_move is not None and self.state.move_start_time is not None:
            ...
            if antennas is None:
                antennas = np.array([-0.1745, 0.1745])  # ~10Ã‚Â° offset
            if body_yaw is None:
                body_yaw = 0.0
    ...
    def stop(self, reset_to_neutral: bool = True) -> None:
        ...
        try:
            neutral_head_pose = create_head_pose(0, 0, 0, 0, 0, 0, degrees=True)
            neutral_antennas = [-0.1745, 0.1745]  # ~10Ã‚Â° offset to reduce shaking
