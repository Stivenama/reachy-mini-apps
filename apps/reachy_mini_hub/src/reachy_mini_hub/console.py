...
from reachy_mini_hub.prompts import get_session_voice, get_session_instructions
from reachy_mini_hub.streaming import AdditionalOutputs, audio_to_float32
from reachy_mini_hub.startup_settings import read_startup_settings, write_startup_settings
from reachy_mini_hub.tools.core_tools import initialize_tools
from reachy_mini_hub.tool_space_routes import register_tool_space_methods
from reachy_mini_hub.personality_routes import (
    build_personality_ops,
    register_personality_methods,
)
from reachy_mini_hub.profile_tool_routes import register_profile_tool_methods
from reachy_mini_hub.audio.startup_config import apply_audio_startup_config
from reachy_mini_hub.conversation_handler import ConversationHandler
from reachy_mini_hub.dance_emotion_moves import EmotionQueueMove
from reachy_mini_hub.timeline import EMOTIONS_DATASET, TimelineController, register_timeline_routes
...
# Local push-to-talk capture cap: ~60s of 16 kHz samples, trimmed from the
# oldest frames when exceeded so memory stays bounded.
MAX_AUDIO_BUFFER_SAMPLES = 16000 * 60
# Minimum hold time before a turn is committed: shorter taps produce too few
# samples for the backend to transcribe, and committing them leaves the robot
# silently unresponsive. ~0.4 s at 16 kHz.
MIN_HOLD_SAMPLES = int(16000 * 0.4)
# RMS por debajo del cual se considera silencio (habla = rms > umbral).
SPEECH_RMS_THRESHOLD = 0.02
# Segundos de silencio tras haber hablado para marcar "listo para responder".
READY_SILENCE_S = 0.9
...
class LocalStream:
    ...
    def __init__(
        ...
        *,
        ...
        self._emotion_library: Optional[RecordedMoves] = None
        self._mic_muted = False  # mic starts live; the UI toggles it via the settings API
        self._listening = False  # push-to-talk: mic is only forwarded while listening
        self._audio_buffer: list[Any] = []  # legacy local buffer (sin uso en streaming)
        self._audio_buffer_samples = 0
        # Estado del streaming en vivo mientras se escucha.
        self._streamed_samples = 0
        self._heard_speech = False
        self._ready_announced = False
        self._last_speech_at: Optional[float] = None
        # Reachy Hub reproduce siempre en el parlante del robot (AB13X externo).
        self._audio_mode = "robot"
    ...
    def _install_handler(self, handler: ConversationHandler) -> None:
        ...
        self.handler.push_to_talk_holding = self._listening
    ...
    def _attach_observers_to_handler(self) -> None:
        ...
        notice_setter = getattr(self.handler, "set_notice_observer", None)
        if callable(notice_setter):
            notice_setter(self._broadcast_notice)
        text_setter = getattr(self.handler, "set_text_observer", None)
        if callable(text_setter):
            text_setter(self._dispatch_text)
    ...
    def _dispatch_text(self, text: str) -> None:
        """Push a conversation.text notification (final assistant text) to JSON-RPC clients."""
        if self._rpc is not None and text:
            self._rpc.broadcast_threadsafe("conversation.text", {"text": text})
    ...
    def _broadcast_audio(self, audio: np.ndarray) -> None:
        """Broadcast one assistant PCM frame to web clients (conversation.audio)."""
        if self._rpc is None or audio.size == 0:
            return
        pcm = base64.b64encode(np.ascontiguousarray(audio, dtype=np.int16).tobytes()).decode("ascii")
        self._rpc.broadcast_threadsafe(
            "conversation.audio",
            {"pcm": pcm, "rate": 16000, "channels": 1},
        )
    ...
    def _dispatch_activity(self, reason: str) -> None:
        ...
        if self._rpc is not None:
            # While the talk button is held the orb must stay on "listening":
            # any activity arriving now belongs to a previous turn still being
            # processed by the backend (the current turn sends no audio until
    ...
    # Push-to-talk state machine. ``_listening`` gates mic frames in
    # record_loop; stopping listening commits the buffered audio so the
    # backend answers immediately instead of waiting for silence.
    def _broadcast_turn(self, state: str, reason: str) -> None:
        """Push a conversation.turn notification if the state actually changed."""
        if state == self._last_turn_state:
            return
        self._last_turn_state = state
        if self._rpc is not None:
            self._rpc.broadcast_threadsafe("conversation.turn", {"state": state, "reason": reason})

    def _set_listening(self, listening: bool) -> None:
        """Toggle the mic gate and mirror the state to motion + JSON-RPC clients."""
        self._listening = bool(listening)
        try:
            self.handler.push_to_talk_holding = self._listening
        except Exception as e:
            logger.debug("Failed to sync push-to-talk state to handler: %s", e)
        try:
            self.handler.deps.movement_manager.set_listening(self._listening)
        except Exception as e:
            logger.debug("Failed to sync listening state to movement manager: %s", e)
        if self._listening:
            self._streamed_samples = 0
            self._heard_speech = False
            self._ready_announced = False
            self._last_speech_at = None
            self._broadcast_turn("listening", "push_to_talk")
            self._emit_phase("listening", "push_to_talk")
        else:
            self._broadcast_turn("thinking", "push_to_talk")
            self._emit_phase("thinking", "push_to_talk")

    def _stop_listening_and_commit(self) -> None:
        """Stop listening and hand the streamed audio to the backend.

        El audio ya se transmitió en vivo mientras escuchaba (record_loop);
        aquí solo se hace commit del buffer del backend y se pide la respuesta.
        Si no se escuchó voz, se avisa en vez de commitear silencio.
        """
        self._set_listening(False)
        if not self._heard_speech or self._streamed_samples < MIN_HOLD_SAMPLES:
            logger.info(
                "Turn without speech (%d samples, heard=%s); skipping commit",
                self._streamed_samples,
                self._heard_speech,
            )
            self._broadcast_notice(
                "turn_too_short",
                "No te escuché. Habla y vuelve a intentarlo.",
            )
            return
        self._ready_announced = False
        if self._asyncio_loop is not None and self._asyncio_loop.is_running():
            asyncio.run_coroutine_threadsafe(self.handler.release_turn([]), self._asyncio_loop)

    def _broadcast_listening_ready(self, ready: bool) -> None:
        """Notify clients that the transcription is ready to respond (or not)."""
        if self._rpc is not None:
            state = "listening_ready" if ready else "listening"
            self._rpc.broadcast_threadsafe("conversation.turn", {"state": state, "reason": "silence"})

    def _broadcast_notice(self, kind: str, message: str) -> None:
        """Push a user-facing conversation.notice notification to JSON-RPC clients."""
        if self._rpc is not None:
            self._rpc.broadcast_threadsafe("conversation.notice", {"kind": kind, "message": message})

    def _fire_timeline_emotion(self, emotion: str) -> None:
        """Queue a recorded emotion move + play its sidecar sound (if any).

        Thread-safe: ``queue_move`` posts to the movement worker's command
        queue, so the timeline can fire from the asyncio thread safely.
        """
        if self._emotion_library is None:
            self._emotion_library = RecordedMoves(EMOTIONS_DATASET)
        try:
            sound_path = self._emotion_library.get(emotion).sound_path
            if sound_path is not None and sound_path.exists():
                self._robot.media.play_sound(str(sound_path))
        except Exception as error:
            logger.debug("Emotion sound unavailable for %s: %s", emotion, error)
        emotion_move = EmotionQueueMove(emotion, self._emotion_library)
        self.handler.deps.movement_manager.queue_move(emotion_move)

    def _stop_timeline_moves(self) -> None:
        """Stop the current emotion (movement + sound) so the idle waiting takes over."""
        self.handler.deps.movement_manager.clear_move_queue()
        try:
            audio = getattr(self._robot.media, "audio", None)
            stop_sound = getattr(audio, "stop_sound", None)
            if callable(stop_sound):
                stop_sound()
        except Exception as error:
            logger.debug("Failed to stop emotion sound: %s", error)
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        # El tile del controlador embebe la UI en un webview que cachea agresivamente;
        # sin esto, una versión vieja de la UI se queda pegada tras los despliegues.
        ...
        if hasattr(settings_app, "mount"):
            try:
                class _NoStoreStaticFiles(StaticFiles):
                    """StaticFiles que fuerza no-cache (el webview del controlador cachea)."""

                    def file_response(self, *args: Any, **kwargs: Any) -> Response:  # type: ignore[override]
                ...
                settings_app.mount(
                    "/static",
                    _NoStoreStaticFiles(directory=str(static_dir)),
                    name="static",
                )
            except Exception:
        ...
        daemon_base_url = os.getenv("REACHY_MINI_DAEMON_URL", "http://127.0.0.1:8000")
        timelines_dir = Path(self._instance_path or Path.home()) / "timelines"
        self._timeline = TimelineController(daemon_base_url, timelines_dir)
        self._timeline.set_motion_hooks(self._fire_timeline_emotion, self._stop_timeline_moves)
        register_timeline_routes(settings_app, self._timeline)
        ...
        # Ã¢â€â‚¬Ã¢â€â‚¬ JSON-RPC control surface (/rpc) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
        ...
        def _rpc_mic(params: dict[str, object]) -> dict[str, object]:
            # Legacy mute API kept for older clients. Muting commits the
            # buffered audio; unmuting does NOT start listening in push-to-talk
            # mode â€” the user controls listening with conversation.listen.
            if "muted" in params:
                if bool(params["muted"]):
                    self._mic_muted = True
                    self._stop_listening_and_commit()
                else:
        ...
        def _rpc_listen(params: dict[str, object]) -> dict[str, object]:
            # Push-to-talk toggle: listen=true opens the mic gate, listen=false
            # closes it and commits the audio so the robot answers now.
            if "listening" in params:
                if bool(params["listening"]):
                    self._mic_muted = False
                    self._set_listening(True)
                else:
                    self._mic_muted = False
                    self._stop_listening_and_commit()
                logger.info("Listening=%s via /rpc", self._listening)
            return {"listening": self._listening}

        @rpc.method("conversation.new_session")  # type: ignore[untyped-decorator]
        async def _rpc_new_session(_params: dict[str, object]) -> dict[str, object]:
            if not self.handler._is_connected():
                raise JsonRpcError("no active session", reason="not_running")
            await self.handler.clear_session()
            self._last_turn_state = None
            rpc.broadcast_threadsafe("conversation.turn", {"state": "idle", "reason": "new_session"})
            rpc.broadcast_threadsafe(
                "conversation.notice", {"kind": "session_reset", "message": "Conversación reiniciada."}
            )
            return {"ok": True}

        @rpc.method("conversation.read")  # type: ignore[untyped-decorator]
        async def _rpc_read(params: dict[str, object]) -> dict[str, object]:
            text = str(params.get("text", "")).strip()
            if not text:
                raise JsonRpcError("text required", reason="invalid_params", code=-32602)
            if not self.handler._is_connected():
                raise JsonRpcError("no active session", reason="not_running")
            self.clear_audio_queue()  # barge in if mid-utterance
            result = await self.handler.read_text(text)
            return {"ok": result == "ok", "mode": result}
        ...
        try:
            ...
            # personalities.* / voices.* over JSON-RPC Ã¢â‚¬â€ the local UI and remote
    ...
    async def _run_handler_startup_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # A fresh backend session starts with an empty audio buffer, so
                # drop any leftover push-to-talk state.
                if self._listening:
                    self._listening = False
                    self._broadcast_turn("ready", "session_ended")
                self._audio_buffer = []
                self._audio_buffer_samples = 0
                if self._restart_requested.is_set():
    ...
    def clear_audio_queue(self) -> None:
        ...
        Calls the SDK's ``clear_player()`` Ã¢â‚¬â€ now a first-class flush on both
        ...
        # Drain the handler's pending output in place Ã¢â‚¬â€ do NOT replace the
    ...
    async def record_loop(self) -> None:
        """Read mic frames and stream them live to the backend while listening.

        El audio va al backend en tiempo real (transcripción parcial en vivo)
        pero sin VAD: el robot nunca responde solo. Un detector de silencio
        local marca el estado "listo para responder" cuando el usuario deja
        de hablar tras haber emitido voz.
        """
        ...
        while not self._stop_event.is_set():
            ...
            if audio_frame is not None and not self._mic_muted and self._listening:
                self._streamed_samples += audio_frame.size
                self._emit_level("user", audio_frame)
                rms = self._frame_rms(audio_frame)
                if rms > SPEECH_RMS_THRESHOLD:
                    self._heard_speech = True
                    self._last_speech_at = time.monotonic()
                    if self._ready_announced:
                        self._ready_announced = False
                        self._broadcast_listening_ready(False)
                elif (
                    self._heard_speech
                    and not self._ready_announced
                    and self._last_speech_at is not None
                    and time.monotonic() - self._last_speech_at > READY_SILENCE_S
                ):
                    self._ready_announced = True
                    self._broadcast_listening_ready(True)
                await self.handler.receive((input_sample_rate, audio_frame))
            await asyncio.sleep(0)  # avoid busy loop

    @staticmethod
    def _frame_rms(audio_frame: Any) -> float:
        """RMS normalizado de un frame de micrófono (0..1)."""
        try:
            samples = audio_to_float32(audio_frame)
            if samples.size == 0:
                return 0.0
    ...
    async def play_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if isinstance(handler_output, AdditionalOutputs):
                for msg in handler_output.args:
                    ...
                    if isinstance(content, str):
                        logger.info(
                            "role=%s content=%s",
                            msg.get("role"),
                            content if len(content) < 500 else content[:500] + "Ã¢â‚¬Â¦",
                        )

            elif isinstance(handler_output, tuple):
                # Never play assistant audio while the talk button is held;
                # the handler also drops it, this is the playback backstop.
                if self._listening:
                    continue
                ...
                # Enviar el audio al navegador (modo "pc"/"both").
                if self._audio_mode in ("pc", "both"):
                    self._broadcast_audio(audio_data)

                # Reproducir en el parlante del robot (modo "robot"/"both").
                if self._audio_mode in ("robot", "both"):
